...
void insert_request(int floor, HardwareOrder orderType, Request *base_req,
                    State *state) {
  Request *current_request = base_req;
  ...
  while (current_request->child != NULL) {
    ...
    bool req_floor_above_current_destination =
        state->going_to_floor < current_request->floor;

    if ((elevator_going_up && !req_floor_above_current_destination &&
         current_request->orderType != HARDWARE_ORDER_DOWN) ||
        (!elevator_going_up && req_floor_above_current_destination &&
         current_request->orderType != HARDWARE_ORDER_UP)) {
      Request *new_req = (Request *)malloc(sizeof(Request));
      if ((new_req != NULL)) {
        new_req->floor = floor;
        new_req->orderType = orderType;
